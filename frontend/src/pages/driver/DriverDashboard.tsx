        ...
        <div style={{ padding: '2rem', textAlign: 'center', maxWidth: '600px', margin: '0 auto' }}>
            ...
            <div 
                ...
                style={{
                    ...
                    margin: '2rem auto', cursor: isToggling ? 'wait' : 'pointer', 
            ...
            <div style={{ marginTop: '2rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {incomingRequests.map((req) => (
                    <div key={req.id} style={{
                        background: 'white', border: '1px solid #ffcccc', borderLeft: '5px solid red', 
                        padding: '1.5rem', borderRadius: '8px', textAlign: 'left',
                        boxShadow: '0 2px 8px rgba(0,0,0,0.05)'
                    }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            <div>
                                <h3 style={{ margin: '0 0 0.5rem 0', color: '#d32f2f' }}>🚨 Emergency Request</h3>
                                <p style={{ margin: 0, color: '#555' }}>📍 {req.pickupAddress || 'Live Location'}</p>
                                <p style={{ margin: '0.2rem 0 0 0', fontSize: '0.9rem', color: '#888' }}>Trip ID: {req.id.substring(0,8)}...</p>
                            </div>
                        ...
                        <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem' }}>
                            <button 
                                ...
                                style={{ flex: 1, padding: '0.8rem', background: '#f5f5f5', border: '1px solid #ccc', borderRadius: '4px', cursor: 'pointer' }}
                            >
                                Ignore
                            </button>
                            <button 
                                onClick={() => handleAccept(req.id)} 
                                style={{ flex: 2, padding: '0.8rem', background: '#28a745', color: 'white', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}
                            >
                                ACCEPT RIDE
                            </button>
                        </div>
